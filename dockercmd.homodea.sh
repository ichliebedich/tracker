...
jq -r --arg TRACKER_DEFAULT_REDIRECT $TRACKER_DEFAULT_REDIRECT '.DefaultRedirect |= $TRACKER_DEFAULT_REDIRECT' $CONFIG_FILE > $TMP_FILE
mv -f $TMP_FILE $CONFIG_FILE

jq -r --arg TRACKER_URL_PREFIX_FILTER $TRACKER_URL_PREFIX_FILTER '.UrlPrefixFilter |= $TRACKER_URL_PREFIX_FILTER' $CONFIG_FILE > $TMP_FILE
mv -f $TMP_FILE $CONFIG_FILE
